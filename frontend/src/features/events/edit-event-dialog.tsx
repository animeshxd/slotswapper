...
export function EditEventDialog({ event, mutation }: { event: Event, mutation: UseMutationResult<Event, Error, { id: number, event: UpdateEventSchema }, unknown> }) {
  const titleId = useId();
  const [title, setTitle] = useState(event.title);
  const [startTime, setStartTime] = useState<Date | undefined>(new Date(event.start_time));
  const [endTime, setEndTime] = useState<Date | undefined>(new Date(event.end_time));
  const [formErrors, setFormErrors] = useState<TreeifyError<UpdateEventSchema> | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedEvent = { title, start_time: startTime.toISOString(), end_time: endTime.toISOString() };
