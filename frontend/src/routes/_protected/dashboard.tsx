...
async function updateEvent({ id, event }: { id: number, event: UpdateEventSchema }): Promise<Event> {
  const res = await fetch(`${import.meta.env.VITE_HTTP_SERVER_URL}/api/events/${id}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
async function updateEventStatus({ id, status }: { id: number, status: string }): Promise<Event> {
  const res = await fetch(`${import.meta.env.VITE_HTTP_SERVER_URL}/api/events/${id}/status`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
        ...
        {events?.map(event => (
          <Card key={event.id}>
            ...
            <CardContent className="grid gap-2">
              ...
              <p className="text-sm"><strong>Status:</strong> {event.status}</p>
              <div className="flex gap-2 mt-4">
                <EditEventDialog event={event} mutation={updateEventMutation} />
                <Button 
                  size="sm" 
                  onClick={() => updateEventStatusMutation.mutate({ id: event.id, status: event.status === 'BUSY' ? 'SWAPPABLE' : 'BUSY' })}
                  disabled={updateEventStatusMutation.isPending || event.status === 'SWAP_PENDING'}
                >
                ...
                <Button 
                  ...
                  disabled={deleteEventMutation.isPending || event.status === 'SWAP_PENDING'}
                >
                  Delete
                </Button>
              </div>
