...
async function fetchSwappableEvents(): Promise<SwappableEvent[]> {
	const res = await fetch(
		`${import.meta.env.VITE_HTTP_SERVER_URL}/api/swappable-slots`,
		{ credentials: "include" },
	);
	if (!res.ok) {
...
async function fetchMySwappableEvents(): Promise<MySwappableEvent[]> {
	const res = await fetch(
		`${import.meta.env.VITE_HTTP_SERVER_URL}/api/events/user?status=SWAPPABLE`,
		{ credentials: "include" },
	);
	if (!res.ok) {
...
async function createSwapRequest(
	...
	const res = await fetch(
		`${import.meta.env.VITE_HTTP_SERVER_URL}/api/swap-request`,
		{
			method: "POST",
	...
	return (
		<div className="p-4 md:p-8">
			...
			<p className="text-muted-foreground mb-4">
				Browse events that other users have made available for swapping.
			</p>
...
function SwapRequestDialog({
	...
	myEvents: MySwappableEvent[] | undefined;
}) {
	const queryClient = useQueryClient();
	const [selectedSlot, setSelectedSlot] = useState<number | null>(null);

	const mutation = useMutation({
		mutationFn: (requesterSlotId: number) =>
			createSwapRequest(requesterSlotId, event.id, event.user_id),
		onSuccess: () => {
		...
		<Dialog>
				...
				<div className="cursor-pointer">
					<Card className="hover:shadow-lg transition-shadow">
						...
						<CardContent className="grid gap-2">
							<p className="text-sm">
								<strong>Owner:</strong> {event.owner_name}
							</p>
							<p className="text-sm">
								<strong>From:</strong>{" "}
			...
			<DialogContent>
				...
				<div className="grid gap-4 py-4">
					<p>
						You are requesting to swap for <strong>{event.title}</strong>.
					</p>
				...
				<DialogFooter>
					...
					<Button
						onClick={() => selectedSlot && mutation.mutate(selectedSlot)}
